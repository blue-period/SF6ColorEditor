...
Below is my attempt at making the changes I outlined in the Plan section of the beginning notes. The part that I want help on is 1). I tried to do just that by taking the SF6 Tools section from the above script and putting it in the display_freecam function as shown below. When I run the script runner in REengine within the game, the script does not seem to run properly. To be more specific, usually when I run the above script through the ScriptRunner, the "Lua FreeCam v1.9.0" dropdown shows up in the Script Generated UI dropdown. And when I run the below script, with the ScriptRunner, nothing shows up in the Script Generated UI dropdown.
...
using the REframework docs, these scripts and the context I provided, tell me your suggestion on what Im doing wrong and if you can provide a solution
